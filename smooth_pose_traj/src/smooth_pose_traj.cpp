...
SmoothPoseTraj::SmoothPoseTraj(const geometry_msgs::PoseArray& input_poses, double point_spacing, bool align_x_to_next, bool flip_direction)
...
bool SmoothPoseTraj::align_x_to_next(geometry_msgs::PoseArray& poses)
{
  ...
  for (size_t i = 0; i < poses.poses.size(); i++)
  {
    ...
    if (flip_direction_)
    {
      R = R * Eigen::AngleAxisd(3.1415, Eigen::Vector3d::UnitZ());
    }
